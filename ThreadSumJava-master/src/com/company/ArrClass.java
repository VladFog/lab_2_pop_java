...
public class ArrClass {
    ...
    public ArrClass(int dim, int threadNum) { //Заповнюєм масив, та рандомне число множемо на -1, щоб потом його
        this.dim = dim;
        arr = new int[dim];
    ...
    public long threadMin(){
        ...
        for (int i=0;i<threadNum-1;i++) {
            threadMins[i] = new ThreadMin(len*i, len*(i+1), this);//Створюємо поток в масиві
            threadMins[i].start();//Запускаємо поток
        }
        threadMins[threadNum-1]= new ThreadMin(len*(threadNum-1), dim, this);//Створюємо послєдній поток в масиві, щоб останні елементи не втрачались
